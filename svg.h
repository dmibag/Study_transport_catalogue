...
struct SolutionColor {
    ...
    std::optional<std::string> operator()(svg::Rgb rgb) const {
        using namespace std::string_literals;
        std::ostringstream out;
        out << "rgb("s << (int) rgb.red << ',' << (int) rgb.green << ',' << (int) rgb.blue << ')';
        return out.str();
    }
    std::optional<std::string> operator()(svg::Rgba rgba) const {
        using namespace std::string_literals;
        std::ostringstream out;
        out << "rgba("s << (int) rgba.red << ',' << (int) rgba.green << ',' << (int) rgba.blue << ',' << rgba.opacity
                << ')';
        return out.str();
    }
};
...
protected:
    ...
    void RenderAttrs(std::ostream &out) const {
        ...
        if (fill_color_) {
            out << " fill=\""sv << *std::visit(SolutionColor { }, *fill_color_) << "\""sv;
        }
        if (stroke_color_) {
            out << " stroke=\""sv << *std::visit(SolutionColor { }, *stroke_color_) << "\""sv;
        }
        if (width_) {
