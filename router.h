...
public:
    explicit Router(DirectedWeightedGraph<double> &graph);
    explicit Router(DirectedWeightedGraph<double> &graph, RoutesInternalData &&routes_internal_data)
...
template <typename Weight>
Router<Weight>::Router(DirectedWeightedGraph<double> &graph)
    : graph_(graph)
    , routes_internal_data_(graph_.GetVertexCount(),
                            std::vector<std::optional<RouteInternalData>>(graph_.GetVertexCount()))
{
...
std::optional<typename Router<Weight>::RouteInfo> Router<Weight>::BuildRoute(VertexId from, VertexId to) const {
    ...
    {
        edges.push_back(graph_.GetParams().at(*edge_id));
    }
